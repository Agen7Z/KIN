    ...
    <div className={`${isOpen ? 'pointer-events-auto' : 'pointer-events-none'} fixed inset-0 z-50`}>
      ...
      >
        ...
        <div className="max-h-[60vh] overflow-y-auto px-5 py-4 divide-y divide-gray-100">
          ...
          {items.map((item) => (
            <div key={item._id} className="py-4 flex gap-3">
              <img 
                src={(item.images && item.images[0]) || item.image} 
                alt={item.name} 
                className="h-16 w-16 object-contain bg-gray-100" 
              />
              <div className="flex-1">
                ...
                <div className="mt-2 flex items-center gap-2">
                  <button 
                    ...
                    className="h-7 w-7 rounded border border-gray-200 text-gray-700 hover:bg-gray-50"
                  >
                    -
                  </button>
        ...
        <div className="mt-auto border-t border-white/30 p-5">
          ...
          {items.length > 0 && (
            <Link 
              to="/checkout" 
              onClick={close}
              className="mt-4 block w-full text-center rounded-md bg-gray-900 px-4 py-2 text-sm font-medium text-white hover:bg-black hover:shadow-md"
            >
              Checkout
            </Link>
          )}
